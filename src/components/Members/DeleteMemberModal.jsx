...
import { remove_member } from '../../redux/actions/membersActions';
export default function DeleteMemberModal({ isOpen, onClose, member }) {
  const dispatch = useDispatch();

  const handleDelete = () => {
    if (member) {
      dispatch(remove_member(member.id));
      onClose();
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-4">حذف العضو</h2>
        <p className="mb-6 text-gray-700">هل أنت متأكد أنك تريد حذف العضو <span className="font-bold">{member.name}</span>؟ لا يمكن التراجع عن هذا الإجراء.</p>
